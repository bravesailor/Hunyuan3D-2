...
import shutil
#added by xlm
def compress2zip(target_folder_name, output_name):
    shutil.make_archive(base_name=output_name, format="zip", root_dir=target_folder_name)
    shutil.rmtree(target_folder_name)

def export_mesh(mesh, save_folder, textured=False):
    if textured:
        output_name ="textured_mesh"
        path = os.path.join(save_folder, f'textured_mesh')
    else:
        output_name = "white_mesh"
        path = os.path.join(save_folder, f'white_mesh')
    #added by xlm, just for test
    os.makedirs(os.path.join(save_folder, f"{output_name}_export"))
    mesh.export(path+".glb", include_normals=textured)

    convert_glb_to_obj(path+".glb", f"{save_folder}/{output_name}_export/{output_name}.obj")
    compress2zip(target_folder_name=f"{save_folder}/{output_name}_export", output_name=f"{save_folder}/{output_name}")
    

    return path+".glb", f"{save_folder}/{output_name}.zip" 
...
def generation_all(
    ...
    path, obj_path = export_mesh(mesh, save_folder, textured=False)
    model_viewer_html = build_model_viewer_html(save_folder, height=596, width=700)

    textured_mesh = texgen_worker(mesh, image)
    path_textured, obj_path_textured = export_mesh(textured_mesh, save_folder, textured=True)
    ...
    return (
        ...
        gr.update(value=obj_path, visible=True),
        gr.update(value=obj_path_textured, visible=True),
        model_viewer_html,
...
def shape_generation(
    ...
    path,_ = export_mesh(mesh, save_folder, textured=False)
    model_viewer_html = build_model_viewer_html(save_folder, height=596, width=700)
...
def build_app():
    ...
    with gr.Blocks(theme=gr.themes.Base(), title='Hunyuan-3D-2.0') as demo:
        ...
        with gr.Row():
            with gr.Column(scale=2):
                ...
                with gr.Group():
                    ...
                    file_out_obj = gr.File(label="File", visible=False)
                    file_out_obj2 = gr.File(label="File", visible=False)
        ...
        btn_all.click(
            ...
            outputs=[file_out, file_out2, file_out_obj, file_out_obj2, html_output1, html_output2],
            queue=True
        ).then(
            lambda: (gr.update(visible=True), gr.update(visible=True),gr.update(visible=True), gr.update(visible=True)),
            outputs=[file_out, file_out2, file_out_obj, file_out_obj2],
        )
    return demo
...
if __name__ == '__main__':
    ...
    demo = build_app()
    demo.queue( max_size=10, api_open=False, default_concurrency_limit=1)
