...
#added by xlm
def compress_obj2zip(target_folder_name, output_name):
    import shutil
    ...
    with gr.Blocks(theme=gr.themes.Base(), title='Hunyuan-3D-2.0', analytics_enabled=False, css=custom_css) as demo:
        ...
        def on_export_click(file_out, file_out2, file_type, reduce_face, export_texture, target_face_num):
            ...
            if export_texture:
                ...
                #modified by xlm, pack all the obj generated files to a zip
                if file_type == 'obj':
                    pack_name = f"obj_{uuid.uuid1()}" 
                    save_folder_obj = os.path.join(save_folder, pack_name)
                    os.makedirs(save_folder_obj)
                    export_mesh(mesh, save_folder_obj, textured=True, type=file_type)
                    # pack_name = f"obj_{uuid.uuid1()}" 
                    compress_obj2zip(target_folder_name=save_folder_obj, output_name=save_folder_obj)
                    path = os.path.join(save_folder, f'{pack_name}.zip')
                else:
                    path = export_mesh(mesh, save_folder, textured=True, type=file_type)
...
if __name__ == '__main__':
    ...
    demo = build_app()
    #added by xlm
    demo.queue(max_size=args.queue_size, api_open=False, default_concurrency_limit=args.concurrency)
